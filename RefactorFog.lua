-- RefactorFog
-- Optional "Show unexplored areas" toggle for the world map: renders every
-- sub-zone overlay texture, including undiscovered ones, so the whole zone
-- map shows without needing to physically explore it first. Off by default.
--
-- Technique and errata data (per-subzone packed texID -> width/height/offset)
-- ported MIT from Leatrix_Maps' FogClear module, itself adapted from
-- Mapster FogClear by Nevcairiel (h.leppkes@gmail.com). Stands down entirely
-- when Leatrix_Maps is loaded -- it ships the same reveal option, and running
-- both would double-hook GetNumMapOverlays/WorldMapFrame_Update.
...
-- No AceDB: metatable so unknown zone keys return {} instead of nil.
setmetatable(errata, {
...
local function FogEnabled()
    return RefactorQoL and RefactorQoL.Get("mapRevealFog")
end

-- Renders every sub-zone overlay for the current zone using the named
-- WorldMapOverlay%d frames -- the same frames Blizzard uses. New frames are
-- created if the errata data needs more than currently exist, updating
-- NUM_WORLDMAP_OVERLAYS so Blizzard's own hide-loop covers them too.
...
local function InitFog()
    if IsAddOnLoaded and IsAddOnLoaded("Leatrix_Maps") then return end
...
-- own -- WorldMapFrame_Update only runs on its usual triggers (opening the
-- map, changing zone, moving). Ticking the box while the map is already open
-- would otherwise look like it does nothing until the next unrelated redraw.
-- Called from Refactor.lua's RefactorQoL.Set when this flag changes.
RefactorFogShared = {
    Redraw = function()
...
if IsAddOnLoaded and IsAddOnLoaded("Blizzard_WorldMap") or WorldMapFrame then
    InitFog()
else
