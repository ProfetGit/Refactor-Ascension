...
A World of Warcraft addon for **[Ascension](https://ascension.gg)**, a custom classless WotLK 3.3.5 server. Refactor smooths out a pile of everyday annoyances: fast auto-loot with animated loot toasts, quest automation, merchant automation, a center-screen crowd-control alert, transmog collection, and more.

> **Looking for the gear comparison?** It now lives in its own addon, **[Refactor Gear](https://github.com/ProfetGit/RefactorGear)** — same feature, none of the tweaks. Install both if you want both; they know about each other (loot toasts still show the upgrade arrow, quest reward auto-pick still works).
...
### 🎁 Loot toasts
Refactor auto-loots instantly, so the stock loot window never shows — animated toasts replace it: item icon, quality-colored name, stack count, and the stack's auction-house value (Auto/TSM/Auctionator). With **Refactor Gear** installed, upgrades also get a pulsing green arrow and the % gain.
...
- **Quest automation** — auto-accept, auto turn-in, gossip/greeting picking, and (with **Refactor Gear** installed) reward auto-pick
...
| `/rfc debug` | Toggle debug output (quest reward auto-pick explains its reasoning) |

Open the config window from the **minimap button** too — left-click to open, drag to reposition.
...
| ![Crowd-control alert](docs/images/cc-alert.png) | ![Minimap button](docs/images/minimap-button.png) |
| **Crowd-control alert** | **Minimap button** |
...
Issues and PRs welcome. Gear-comparison issues belong on the [Refactor Gear](https://github.com/ProfetGit/RefactorGear) repo.
